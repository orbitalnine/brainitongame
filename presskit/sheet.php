...
else
{
	for( $i = 0; $i < count($trailers); $i++ )
	{
		...
		<iframe src="http://www.youtube.com/embed/'. $youtube .'?rel=0" frameborder="0" allowfullscreen></iframe>
</div>';
			} elseif ( $ytfirst == 0 ) {
				echo '<div class="uk-responsive-width iframe-container">
...
for( $i = 0; $i < count($additionals); $i++ )
{
	...
	{		if( $child->getName() == "linktitle" ) {			$linkTitle = $child;		} else if( $child->getName() == "title" ) {
			$title = $child;
